...
import { ArrowDown, Mail, MapPin, BookOpen, Sparkles, GraduationCap, ArrowRight } from 'lucide-react';

const Hero = () => {
  const highlights = [
    {
      title: "Academic Focus",
      desc: "Deep engagement with classical literature, linguistic analysis, and modern textual criticism.",
      path: "/about",
      icon: BookOpen,
      accent: "text-literary-600 bg-literary-50 border-literary-200"
    },
    {
      title: "Digital & Language Agility",
      desc: "Fluent multilingual communication combined with essential administrative computer foundations.",
      path: "/skills",
      icon: Sparkles,
      accent: "text-literary-500 bg-literary-50 border-literary-200"
    },
    {
      title: "Career Goals",
      desc: "Seeking collaborative, professional environments where structured storytelling and clear documentation matter.",
      path: "/contact",
      icon: GraduationCap,
      accent: "text-literary-700 bg-literary-50 border-literary-200"
    }
  ];

  return (
    <div className="pt-20 pb-16">
      <section id="home" className="min-h-[calc(100vh-160px)] flex items-center relative overflow-hidden pb-12">
      ...
      {/* Embedded Featured Highlights Cards Section (More Info) */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4 border-t border-literary-200/60 pt-12">
        <div className="text-center mb-10">
          <h2 className="text-xs uppercase tracking-widest font-bold text-literary-500 mb-2">Explore the Portals</h2>
          <p className="text-xl font-serif text-ink">Dedicated Subject Contexts</p>
        </div>
          ...
          {highlights.map((item, idx) => {
            ...
            return (
              <motion.div
                ...
                className="bg-white/80 backdrop-blur-xs rounded-2xl p-6 border border-literary-200 shadow-2xs hover:shadow-md transition-all flex flex-col justify-between group"
              >
                <div>
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 border ${item.accent}`}>
                    <IconComp className="w-6 h-6" />
                  </div>
                  <h3 className="font-serif font-bold text-lg text-ink mb-2 group-hover:text-literary-600 transition-colors">
                    {item.title}
                  </h3>
                ...
                <Link
                  to={item.path}
                  className="inline-flex items-center gap-2 text-xs font-semibold text-literary-600 hover:text-literary-700 transition-colors w-fit group/link"
                >
                  View Details 
